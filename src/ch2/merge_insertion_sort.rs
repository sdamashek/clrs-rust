pub fn insertion_sort<T: PartialOrd>(v: &mut Vec<T>, p: usize, r: usize) {
    // p, r inclusive 
    for j in (p+1)..(r+1) {
        let key = v.remove(j);
...
fn merge_sort_sentinel<T: Ord + Clone>(A: &mut Vec<T>, p: usize, r: usize, sentinel: &T, k: usize) {
    // p, r inclusive
    if p < r {
        if r - p < k {
            ...
            let q = p + (r - p) / 2;
            merge_sort_sentinel(A, p, q, sentinel, k);
            merge_sort_sentinel(A, q + 1, r, sentinel, k);
...
pub fn merge_sort<T: Ord + Clone>(A: &mut Vec<T>, p: usize, r: usize) {
    ...
    let k = ((r - p) as f64).log2() as usize; // below k, it is more efficient to use insertion sort.

    if let Some(max) = sentinel {
        merge_sort_sentinel(A, p, r, &max, k);
    }
}
